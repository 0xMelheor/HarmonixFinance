...
// File contracts/HarmonixKey.sol
...
contract HarmonixKey is ERC721Enumerable, Dictionary, Ownable {
    ...
    function isVisible(uint keyID) public view returns (bool) {
    ...
    // hide vault data from outsiders (only NFT owner can set this)
    function activate(uint keyID) external {
        if (msg.sender == IERC721(address(this)).ownerOf(keyID)) {
            tokens[keyID].active = true;
        }
    }

    // show vault data to outsiders (only NFT owner can set this)
    function deactivate(uint keyID) external {
        if (msg.sender == IERC721(address(this)).ownerOf(keyID)) {
            tokens[keyID].active = false;
        }
    }

    // reveals whether the key data is visible or not (only the vault and key owner can see this)
    function isActive(uint keyID) public view returns (bool) {
        if (msg.sender == IERC721(address(this)).ownerOf(keyID) || msg.sender == owner()) {
